...
# Enhanced error checking for data loading
if(inherits(spectra, "try-error") || !is.list(spectra) || identical(names(spectra), "NONE")){
...
if(inherits(canopyFoliage, "try-error") || !is.list(canopyFoliage) || identical(names(canopyFoliage), "NONE")){
...
```{r dup check results}
print(paste0('The field tables have ', sum(dup_summary$Duplicates), ' total duplicates across all tables.'))
```

### Duplicate Summary {.tabset}
...
fsp_complete_bout_lut <- try(read.csv("fsp_complete_bout.csv", stringsAsFactors = FALSE), silent = TRUE)

if(inherits(fsp_complete_bout_lut, "try-error")){
  # If lookup table not found, create empty results
  fsp.bout.completeness <- list(
    complete_bout_all = data.frame(),
    complete_bout_flags = data.frame(),
    complete_bout_summary = data.frame(
      Check = "Bout completeness (eventID to boutID match)",
      Status = "Lookup table not available"
    )
  )
} else {
  ...
  fsp.bout.completeness.check <- fsp_boutMetadata_noDups %>%
    ...
    mutate(
      boutCompleteness = ifelse(is.na(year), "Missing from lookup", "Found in lookup")
    )
...
if(nrow(fsp.bout.completeness$complete_bout_summary) > 0){
  DT::datatable(fsp.bout.completeness$complete_bout_summary, options = dtOptions)
} else {
  cat("Bout completeness check could not be performed")
}
```
...
```{r bout complete all}
if(nrow(fsp.bout.completeness$complete_bout_all) > 0){
  DT::datatable(fsp.bout.completeness$complete_bout_all, options = dtOptions)
} else {
  cat("No bout data available")
}
```
...
```{r within rec summary}
summary_df <- rbind(
  fsp.boutMetadata.within.rec$summary,
  fsp.sampleMetadata.within.rec$summary,
  fsp.spectralData.within.rec$summary
)
DT::datatable(summary_df, options = dtOptions)
```

### Band count completeness {.tabset}
...
if("downloadFileUrl" %in% names(fsp_spectralData_pub) && any(!is.na(fsp_spectralData_pub$downloadFileUrl))){
  ...
  for(i in 1:nrow(download_urls)){ # Process ALL files
    tryCatch({
      # Download CSV
...
```{r band count summary}
DT::datatable(fsp.band.count.completeness$summary, options = dtOptions)
```
...
```{r fsp cfc timing summary}
DT::datatable(fsp.cfc.timing.check$summary, options = dtOptions)
```

#### All Events

```{r fsp cfc timing all}
...
```{r fsp cfc timing flags}
if(nrow(fsp.cfc.timing.check$flagged) > 0){
  DT::datatable(fsp.cfc.timing.check$flagged, options = dtOptions)
} else {
  cat("All events within 31-day timing window")
}
```

### Bout duration timeliness {.tabset}
...
### Reflectance range check {.tabset}
...
DT::datatable(fsp.reflectance.range$summary, options = dtOptions)
```

#### All Files

```{r reflectance range all}
...
```{r wavelength range flags}
if(nrow(fsp.wavelength.range$flagged) > 0){
  DT::datatable(fsp.wavelength.range$flagged, options = dtOptions)
} else {
  cat("All wavelength values within 300-2600 range")
}
```

### Spectral ratio validation {.tabset}
...
summary_all_checks <- data.frame(
  ...
  Issues_Found = c(
    ...
    ifelse(exists("fsp.bout.completeness"), nrow(fsp.bout.completeness$complete_bout_flags), NA),
...
report_metadata <- data.frame(
  ...
  Value = c(Sys.Date(), startDate_checked, endDate_checked, scriptType, "v5 - Production Ready")
)
```

```{r final summary}
DT::datatable(summary_all_checks, options = dtOptions, caption = "Summary of all QC checks performed")
```
