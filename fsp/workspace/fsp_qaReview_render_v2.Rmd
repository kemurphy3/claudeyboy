...
# Load GitHub PAT from .Renviron file
github_pat <- Sys.getenv('GITHUB_PAT')
if (github_pat == "") stop("GitHub PAT not found in .Renviron file! Make sure GITHUB_PAT is set in your main Git folder's .Renviron file.")
...
canopyFoliage <- try(neonUtilities::loadByProduct(
  ...
  release = "LATEST",
  token = github_pat,
  silent = T)) # Using the github_pat variable loaded from .Renviron
...
```{r download_ingest, include=FALSE}
ingest_url <- "https://api.github.com/repos/NEONScience/definitional-data/contents/ingests/fsp_dataingest.txt"
req <- httr::GET(ingest_url, httr::add_headers(Authorization = paste('token', github_pat)))
stop_for_status(req)
meta <- httr::content(req, as = "parsed")
download_url <- meta$download_url
fsp_dataingest <- read.delim(download_url, stringsAsFactors = FALSE)
get_required_fields <- function(table_name) {
  fsp_dataingest$fieldName[stringr::str_detect(fsp_dataingest$entryValidationRulesForm, "\\[REQUIRE\\]") & fsp_dataingest$tableName == table_name]
}
required_fields_sample <- get_required_fields("fsp_sampleMetadata_in")
required_fields_bout <- get_required_fields("fsp_boutMetadata_in")
required_fields_spectral <- get_required_fields("fsp_spectralData_in")
```
...
DT::datatable(fsp1$complete_bout_pipeline_summary, 
...
if(nrow(fsp1$complete_bout_pipeline_flags) > 0){
  DT::datatable(fsp1$complete_bout_pipeline_flags,
...
```{r completeness_checks, eval=TRUE}
# sample_meta, bout_meta, spectral_data assumed loaded from NEON API
missing_fields_sample <- sapply(required_fields_sample, function(f) sum(is.na(sample_meta[[f]]) | sample_meta[[f]] == ""))
missing_fields_sample <- missing_fields_sample[missing_fields_sample > 0]
if (length(missing_fields_sample) > 0) {
  print("Missing values in required fields (sampleMetadata):")
  print(missing_fields_sample)
} else {
  print("All required fields in sampleMetadata are filled.")
}

missing_fields_bout <- sapply(required_fields_bout, function(f) sum(is.na(bout_meta[[f]]) | bout_meta[[f]] == ""))
missing_fields_bout <- missing_fields_bout[missing_fields_bout > 0]
if (length(missing_fields_bout) > 0) {
  print("Missing values in required fields (boutMetadata):")
  print(missing_fields_bout)
} else {
  print("All required fields in boutMetadata are filled.")
}

missing_fields_spectral <- sapply(required_fields_spectral, function(f) sum(is.na(spectral_data[[f]]) | spectral_data[[f]] == ""))
missing_fields_spectral <- missing_fields_spectral[missing_fields_spectral > 0]
if (length(missing_fields_spectral) > 0) {
  print("Missing values in required fields (spectralData):")
  print(missing_fields_spectral)
} else {
  print("All required fields in spectralData are filled.")
}
```
#### Summary, plots
``` {r within bout completeness summary table 1}
DT::datatable(bbc.2.plots$complete_within_bout_pipeline_summary, 
...
if(nrow(bbc.2.plots$complete_within_bout_pipeline_flags) > 0){
  DT::datatable(bbc.2.plots$complete_within_bout_pipeline_flags,
...
DT::datatable(bbc.2.records$complete_within_bout_pipeline_summary, 
...
if(nrow(bbc.2.records$complete_within_bout_pipeline_flags) > 0){
  DT::datatable(bbc.2.records$complete_within_bout_pipeline_flags,
...
| :----: |:----:|:----:| :----:|:----:|
| CLBJ_001-M-14-4-20190416  | 2019-04-16| 20.9 | 0.17 | 4.47 |

Transforms into three rows in long-format:

| sampleID| collectDate|measurement|value|
| :----: |:----:|:----:| :----:|:----:|
| CLBJ_001-M-14-4-20190416| 2019-04-16| CNratio | 20.9 | 
...
```{r fsp_complete_bout, eval=TRUE}
# Download lookup table from GitHub API if not available locally
if (!file.exists('fsp_complete_bout.csv')) {
  # Try to download from GitHub API
  lut_url <- "https://api.github.com/repos/NEONScience/os-data-quality-review/contents/qc_lookup_tables/fsp_complete_bout.csv"
  req <- httr::GET(lut_url, httr::add_headers(Authorization = paste('token', github_pat)))
  if (httr::status_code(req) == 200) {
    meta <- httr::content(req, as = "parsed")
    download_url <- meta$download_url
    fsp_bout_lut <- read.csv(download_url, stringsAsFactors = FALSE)
  } else {
    # Create a basic lookup table structure if GitHub download fails
    fsp_bout_lut <- data.frame(
      year = unique(format(as.Date(fsp_boutMetadata_noDups$collectDate), "%Y")),
      expectedBouts = 1,  # Default expectation - adjust as needed
      stringsAsFactors = FALSE
    )
  }
} else {
  fsp_bout_lut <- read.csv('fsp_complete_bout.csv', stringsAsFactors = FALSE)
}

# Run complete_bout check
if (exists('fsp_boutMetadata_noDups') && nrow(fsp_boutMetadata_noDups) > 0) {
  fsp_bout_check <- neonOSqc::complete_bout(
    input.df = fsp_boutMetadata_noDups,
    mod = 'fsp',
    date.var = 'collectDate',
    bout.var = 'eventID',
  ...
  print("Bout completeness check completed")
} else {
  print("No bout metadata available for completeness check")
}
```

# 2. Completeness: Within-bout completeness (bands per sample)
```{r fsp_within_bout, eval=TRUE}
# Download lookup table from GitHub API if not available locally
if (!file.exists('fsp_within_bout_nums.csv')) {
  # Try to download from GitHub API
  lut_url <- "https://api.github.com/repos/NEONScience/os-data-quality-review/contents/qc_lookup_tables/fsp_within_bout_nums.csv"
  req <- httr::GET(lut_url, httr::add_headers(Authorization = paste('token', github_pat)))
  if (httr::status_code(req) == 200) {
    meta <- httr::content(req, as = "parsed")
    download_url <- meta$download_url
    fsp_within_bout_lut <- read.csv(download_url, stringsAsFactors = FALSE)
  } else {
    # Create a basic lookup table structure if GitHub download fails
    fsp_within_bout_lut <- data.frame(
      eventID = unique(fsp_sampleMetadata_noDups$eventID),
      expectedSamples = 1,  # Default expectation - adjust as needed
      stringsAsFactors = FALSE
    )
  }
} else {
  fsp_within_bout_lut <- read.csv('fsp_within_bout_nums.csv', stringsAsFactors = FALSE)
}

# Run complete_within_bout check
if (exists('fsp_sampleMetadata_noDups') && nrow(fsp_sampleMetadata_noDups) > 0) {
  fsp_within_bout_check <- neonOSqc::complete_within_bout(
    input.df = fsp_sampleMetadata_noDups,
    mod = 'fsp',
    event.var = 'eventID',
    table.name = 'fsp_sampleMetadata',
    type = 'samples per bout',
    lut = fsp_within_bout_lut
  )
  print("Within-bout completeness check completed")
} else {
  print("No sample metadata available for within-bout check")
}
```

# 3. Completeness: All required fields filled (within record completeness)
```{r fsp_within_rec_bout, eval=TRUE}
# Run complete_within_rec check for bout metadata
if (exists('fsp_boutMetadata_noDups') && nrow(fsp_boutMetadata_noDups) > 0) {
  fsp_within_rec_bout_check <- neonOSqc::complete_within_rec(
    input.df = fsp_boutMetadata_noDups,
    id.var = 'eventID',
    required.fields = required_fields_bout
  )
  print("Within-record completeness check for bout metadata completed")
} else {
  print("No bout metadata available for within-record check")
}

# Run complete_within_rec check for sample metadata
if (exists('fsp_sampleMetadata_noDups') && nrow(fsp_sampleMetadata_noDups) > 0) {
  fsp_within_rec_sample_check <- neonOSqc::complete_within_rec(
    input.df = fsp_sampleMetadata_noDups,
    id.var = 'sampleID',
    required.fields = required_fields_sample
  )
  print("Within-record completeness check for sample metadata completed")
} else {
  print("No sample metadata available for within-record check")
}
```

# 4. Completeness: Cross-table completeness (parent/child relationships)
```{r fsp_cross_table, eval=TRUE}
# Check parent/child relationship between sample metadata and spectral data
if (exists('fsp_sampleMetadata_noDups') && exists('fsp_spectralData_noDups')) {
  fsp_cross_table_check <- neonOSqc::complete_cross_table(
    parent.input.df = fsp_sampleMetadata_noDups,
    child.input.df = fsp_spectralData_noDups,
    parent.id.var = 'sampleID',
    child.id.var = 'sampleID',
    exact.id.match = TRUE
  )
  print("Cross-table completeness check completed")
} else {
  print("Missing required data for cross-table completeness check")
}

# Check that every eventID in bout metadata has at least one sample
if (exists('fsp_boutMetadata_noDups') && exists('fsp_sampleMetadata_noDups')) {
  bout_eventIDs <- unique(fsp_boutMetadata_noDups$eventID)
  sample_eventIDs <- unique(fsp_sampleMetadata_noDups$eventID)
  missing_events <- setdiff(bout_eventIDs, sample_eventIDs)
  
  cat("Cross-table check: EventID coverage\n")
  cat("Total eventIDs in bout metadata:", length(bout_eventIDs), "\n")
  cat("EventIDs with samples:", length(intersect(bout_eventIDs, sample_eventIDs)), "\n")
  cat("EventIDs missing samples:", length(missing_events), "\n")
  
  if (length(missing_events) > 0) {
    print("EventIDs without samples:")
    print(missing_events)
  }
}

# Check that FSP sampleIDs match CFC collection list
if (exists('canopyFoliage') && !is.null(canopyFoliage) && 'cfc_fieldData' %in% names(canopyFoliage)) {
  cfc_sampleIDs <- unique(canopyFoliage$cfc_fieldData$sampleID)
  fsp_sampleIDs <- unique(fsp_sampleMetadata_noDups$sampleID)
  
  # Find FSP samples that are not in CFC collection list
  fsp_not_in_cfc <- setdiff(fsp_sampleIDs, cfc_sampleIDs)
  fsp_in_cfc <- intersect(fsp_sampleIDs, cfc_sampleIDs)
  
  cat("\nCFC sampleID matching check:\n")
  cat("Total FSP samples:", length(fsp_sampleIDs), "\n")
  cat("FSP samples found in CFC collection:", length(fsp_in_cfc), "\n")
  cat("FSP samples NOT in CFC collection:", length(fsp_not_in_cfc), "\n")
  
  if (length(fsp_not_in_cfc) > 0) {
    cat("\nFSP sampleIDs not found in CFC collection list:\n")
    print(fsp_not_in_cfc)
  }
  
  # Check for CFC samples that don't have corresponding FSP data
  cfc_not_in_fsp <- setdiff(cfc_sampleIDs, fsp_sampleIDs)
  if (length(cfc_not_in_fsp) > 0) {
    cat("\nCFC sampleIDs without corresponding FSP data:", length(cfc_not_in_fsp), "\n")
    # Only show first 10 to avoid overwhelming output
    if (length(cfc_not_in_fsp) <= 10) {
      print(cfc_not_in_fsp)
    } else {
      print(paste("First 10:", paste(head(cfc_not_in_fsp, 10), collapse = ", ")))
    }
  }
} else {
  cat("CFC data not available for sampleID matching check\n")
}
```

# 5. Timeliness: Bout completed within 31 days of CFC collection start date
```{r fsp_timely_bout_duration, eval=TRUE}
# Use CFC data already loaded as canopyFoliage
if (exists('canopyFoliage') && !is.null(canopyFoliage)) {
  # Extract CFC collection dates from the canopyFoliage data
  # Assuming we need site and collection date from CFC data
  if ('cfc_fieldData' %in% names(canopyFoliage)) {
    cfc_data <- canopyFoliage$cfc_fieldData
    
    # Create reference dates for comparison
    # We need: siteID, collectDate, and possibly sampleID for matching
    cfc_reference_dates <- cfc_data %>%
      ...
      dplyr::summarize(
        cfc_start_date = min(collectDate, na.rm = TRUE),
        cfc_end_date = max(collectDate, na.rm = TRUE),
        .groups = 'drop'
      )
    
    # Now compare FSP bout dates to CFC collection dates
    if (exists('fsp_sampleMetadata_noDups') && nrow(fsp_sampleMetadata_noDups) > 0) {
      fsp_bout_dates <- fsp_sampleMetadata_noDups %>%
        dplyr::select(siteID, eventID, collectDate) %>%
        dplyr::mutate(
          collectDate = as.Date(collectDate),
          year = format(collectDate, "%Y")
        )
      
      # Join FSP dates with CFC reference dates
      comparison_data <- fsp_bout_dates %>%
        dplyr::left_join(cfc_reference_dates, by = c("siteID", "year")) %>%
        dplyr::mutate(
          days_from_cfc_start = as.numeric(collectDate - cfc_start_date),
          within_31_days = abs(days_from_cfc_start) <= 31
        )
      
      # Summary results
      cat("CFC-FSP timing check results:\n")
      cat("Total FSP bouts checked:", nrow(comparison_data), "\n")
      cat("Bouts within 31 days of CFC start:", sum(comparison_data$within_31_days, na.rm = TRUE), "\n")
      cat("Bouts outside 31-day window:", sum(!comparison_data$within_31_days, na.rm = TRUE), "\n")
      
      # Flag problematic bouts
      problematic_bouts <- comparison_data %>%
        dplyr::filter(!within_31_days | is.na(within_31_days))
      
      if (nrow(problematic_bouts) > 0) {
        cat("\nBouts outside 31-day window or missing CFC reference:\n")
        print(problematic_bouts[, c("siteID", "eventID", "collectDate", "cfc_start_date", "days_from_cfc_start")])
      }
      
      print("CFC-FSP timing check completed")
    } else {
      cat("No FSP sample metadata available for timing check\n")
    }
  } else {
    cat("CFC field data not found in canopyFoliage\n")
  }
} else {
  cat("No CFC data (canopyFoliage) available for timing check\n")
}
```

# 6. Plausibility: Duplicate checks (already implemented in earlier section)
```{r fsp_duplicates_review, eval=TRUE}
# Review duplicate check results from earlier section
if (exists('fsp_boutMetadata_dupCheck')) {
  cat("Bout metadata duplicate check results:\n")
  print(table(fsp_boutMetadata_dupCheck$duplicateRecordQF))
}

if (exists('fsp_sampleMetadata_dupCheck')) {
  cat("Sample metadata duplicate check results:\n")
  print(table(fsp_sampleMetadata_dupCheck$duplicateRecordQF))
}

if (exists('fsp_spectralData_dupCheck')) {
  cat("Spectral data duplicate check results:\n")
  print(table(fsp_spectralData_dupCheck$duplicateRecordQF))
}
```

# 7. Plausibility: Reflectance and wavelength range checks (enhanced)
```{r fsp_spectral_range_checks, eval=TRUE}
# Process all spectral data files from the nested structure
if (exists('fsp_spectralData') && 'downloadFileURL' %in% colnames(fsp_spectralData)) {
  reflectance_out_of_range_all <- list()
  wavelength_out_of_range_all <- list()
  
  for (i in seq_len(nrow(fsp_spectralData))) {
    file_url <- fsp_spectralData$downloadFileURL[i]
    ...
    tryCatch({
      ...
      if ("reflectance" %in% colnames(spec_df)) {
        out_of_range <- which(spec_df$reflectance < 0 | spec_df$reflectance > 1)
        if (length(out_of_range) > 0) {
          reflectance_out_of_range_all[[sample_id]] <- spec_df[out_of_range, c("wavelength", "reflectance")]
        }
      }
      
      # Check wavelength range (300-2600)
      if ("wavelength" %in% colnames(spec_df)) {
        out_of_range <- which(spec_df$wavelength < 300 | spec_df$wavelength > 2600)
        if (length(out_of_range) > 0) {
          wavelength_out_of_range_all[[sample_id]] <- spec_df[out_of_range, c("wavelength", "reflectance")]
        }
      }
    }, error = function(e) {
      cat("Error processing file for sample", sample_id, ":", e$message, "\n")
    })
    unlink(temp)
  }
  
  # Summary results
  cat("Spectral range check results:\n")
  cat("Samples with out-of-range reflectance:", length(reflectance_out_of_range_all), "\n")
  cat("Samples with out-of-range wavelength:", length(wavelength_out_of_range_all), "\n")
  
  if (length(reflectance_out_of_range_all) > 0) {
    print("Samples with reflectance issues:")
    print(names(reflectance_out_of_range_all))
  }
  
  if (length(wavelength_out_of_range_all) > 0) {
    print("Samples with wavelength issues:")
    print(names(wavelength_out_of_range_all))
  }
} else {
  cat("No spectral data with downloadFileURL found\n")
}
```

# 8. Timeliness: Length of time between first and last FSP sample ≤ 30 days
```{r fsp_sample_time_range, eval=TRUE}
# Process all spectral data files to check time ranges
if (exists('fsp_spectralData') && 'downloadFileURL' %in% colnames(fsp_spectralData)) {
  sample_time_ranges <- list()
  ...
  for (i in seq_len(nrow(fsp_spectralData))) {
    ...
    tryCatch({
      ...
      # Extract date from spectralSampleID or filename
      if ("spectralSampleID" %in% colnames(spec_df)) {
        # Try to extract date from spectralSampleID format
        date_match <- stringr::str_extract(spec_df$spectralSampleID[1], '\\d{8}')
        if (!is.na(date_match)) {
          sample_date <- as.Date(date_match, format='%Y%m%d')
          sample_time_ranges[[sample_id]] <- sample_date
        }
      }
    }, error = function(e) {
      cat("Error processing time range for sample", sample_id, ":", e$message, "\n")
    })
    unlink(temp)
  }
  
  # Calculate time ranges
  if (length(sample_time_ranges) > 0) {
    dates <- unlist(sample_time_ranges)
    date_range <- range(dates, na.rm = TRUE)
    days_between <- as.numeric(date_range[2] - date_range[1])
    
    cat("FSP sample time range check:\n")
    cat("First sample date:", as.character(date_range[1]), "\n")
    cat("Last sample date:", as.character(date_range[2]), "\n")
    cat("Days between first and last sample:", days_between, "\n")
    
    if (days_between > 30) {
      cat("WARNING: Time range exceeds 30 days\n")
    } else {
      cat("Time range is within acceptable limits (≤ 30 days)\n")
    }
  } else {
    cat("No valid dates found in spectral data\n")
  }
} else {
  cat("No spectral data available for time range check\n")
}
```

# 9. Completeness: n > 1 per eventID (sample-bout correspondence)
```{r fsp_n_gt_1_per_eventID, eval=TRUE}
# Check that each eventID has at least one sample
if (exists('fsp_boutMetadata_noDups') && exists('fsp_sampleMetadata_noDups')) {
  # Count samples per eventID
  event_counts <- table(fsp_sampleMetadata_noDups$eventID)
  bout_eventIDs <- unique(fsp_boutMetadata_noDups$eventID)
  
  # Find eventIDs with no samples
  missing_events <- bout_eventIDs[!bout_eventIDs %in% names(event_counts)]
  
  cat("Sample-bout correspondence check:\n")
  cat("Total eventIDs in bout metadata:", length(bout_eventIDs), "\n")
  cat("EventIDs with at least one sample:", sum(bout_eventIDs %in% names(event_counts)), "\n")
  cat("EventIDs with NO samples:", length(missing_events), "\n")
  
  if (length(missing_events) > 0) {
    print("EventIDs missing samples:")
    print(missing_events)
  }
  
  # Check for eventIDs with multiple samples
  multiple_samples <- event_counts[event_counts > 1]
  cat("EventIDs with multiple samples:", length(multiple_samples), "\n")
  
  if (length(multiple_samples) > 0) {
    print("EventIDs with multiple samples:")
    print(multiple_samples)
  }
} else {
  cat("Missing required data for sample-bout correspondence check\n")
}
```

# 10. Plausibility: Spectra ratio check (reflectance at ~1000 nm > ~500 nm)
```{r fsp_spectra_ratio_check, eval=TRUE}
# Process all spectral data files for ratio checks
if (exists('fsp_spectralData') && 'downloadFileURL' %in% colnames(fsp_spectralData)) {
  ...
  for (i in seq_len(nrow(fsp_spectralData))) {
    ...
    tryCatch({
      ...
      if ("wavelength" %in% colnames(spec_df) && "reflectance" %in% colnames(spec_df)) {
        # Use a window of +/- 5 nm around 500 and 1000
        window_500 <- 495:505
        window_1000 <- 995:1005
        
        # Calculate average reflectance in each window
        avg_500 <- mean(spec_df$reflectance[spec_df$wavelength %in% window_500], na.rm = TRUE)
        avg_1000 <- mean(spec_df$reflectance[spec_df$wavelength %in% window_1000], na.rm = TRUE)
        
        ratio_results_all[[sample_id]] <- data.frame(
          avg_500 = avg_500,
          avg_1000 = avg_1000,
          ratio_check = avg_1000 > avg_500,
          stringsAsFactors = FALSE
        )
      }
    }, error = function(e) {
      cat("Error processing ratio check for sample", sample_id, ":", e$message, "\n")
    })
    unlink(temp)
  }
  ...
  if (length(ratio_results_all) > 0) {
    ratio_df <- do.call(rbind, ratio_results_all)
    ratio_df$spectralSampleID <- names(ratio_results_all)
    
    n_failed <- sum(!ratio_df$ratio_check, na.rm = TRUE)
    cat("Spectra ratio check results:\n")
    cat("Total samples checked:", nrow(ratio_df), "\n")
    cat("Samples where 1000nm > 500nm:", sum(ratio_df$ratio_check, na.rm = TRUE), "\n")
    cat("Samples where 1000nm ≤ 500nm:", n_failed, "\n")
    
    if (n_failed > 0) {
      print("Samples failing ratio check:")
      print(ratio_df[!ratio_df$ratio_check, c("spectralSampleID", "avg_500", "avg_1000")])
    }
  } else {
    cat("No spectral data processed for ratio check\n")
  }
} else {
  cat("No spectral data available for ratio check\n")
}
```

# --- END QAQC CHECKS ---

# --- SPECTRAL DATA: Band count, reflectance, and wavelength checks ---
```{r spectral_data_band_reflectance_wavelength_checks, eval=TRUE}
# Process all spectral data files for comprehensive checks
if (exists('fsp_spectralData') && 'downloadFileURL' %in% colnames(fsp_spectralData)) {
  band_count_results <- list()
  reflectance_range_results <- list()
  wavelength_range_results <- list()
  ...
  for (i in seq_len(nrow(fsp_spectralData))) {
    ...
    tryCatch({
      ...
      # Band count check (should be 10755 +/- 5)
      n_bands <- nrow(spec_df)
      band_count_results[[sample_id]] <- n_bands
      
      # Reflectance range check (0-1)
      if ("reflectance" %in% colnames(spec_df)) {
        out_of_range <- which(spec_df$reflectance < 0 | spec_df$reflectance > 1)
        if (length(out_of_range) > 0) {
          reflectance_range_results[[sample_id]] <- spec_df[out_of_range, c("wavelength", "reflectance")]
        }
      }
      ...
      if ("wavelength" %in% colnames(spec_df)) {
        out_of_range <- which(spec_df$wavelength < 300 | spec_df$wavelength > 2600)
        if (length(out_of_range) > 0) {
          wavelength_range_results[[sample_id]] <- spec_df[out_of_range, c("wavelength", "reflectance")]
        }
      }
    }, error = function(e) {
      band_count_results[[sample_id]] <- NA
      reflectance_range_results[[sample_id]] <- NA
      wavelength_range_results[[sample_id]] <- NA
  ...
  # Band count summary
  band_count_df <- data.frame(
    spectralSampleID = names(band_count_results),
    n_bands = unlist(band_count_results),
    stringsAsFactors = FALSE
  )
  band_count_flags <- band_count_df[abs(band_count_df$n_bands - 10755) > 5, ]
  
  cat("\n=== SPECTRAL DATA COMPREHENSIVE CHECKS ===\n")
  cat("\nBand count check (should be 10755 +/- 5):\n")
  if (nrow(band_count_flags) > 0) {
    print("Samples with unexpected band counts:")
    print(band_count_flags)
  } else {
    print("All spectra have expected number of bands.")
  }
  
  # Reflectance range summary
  cat("\nReflectance range check (should be 0-1):\n")
  if (length(Filter(Negate(is.null), reflectance_range_results)) > 0) {
    print("Spectra with out-of-range reflectance values:")
    print(names(Filter(Negate(is.null), reflectance_range_results)))
  } else {
    print("All reflectance values are within 0-1.")
  }
  
  # Wavelength range summary
  cat("\nWavelength range check (should be 300-2600):\n")
  if (length(Filter(Negate(is.null), wavelength_range_results)) > 0) {
    print("Spectra with out-of-range wavelength values:")
    print(names(Filter(Negate(is.null), wavelength_range_results)))
  } else {
    print("All wavelength values are within 300-2600.")
  }
} else {
  cat("No spectral data available for comprehensive checks\n")
}
```

# --- Row 17 placeholder ---
# TODO: Implement spectra ratio by biome/NLCD class type when supporting data is available.
# This requires biome/NLCD classification data for each sample location
# File path needed: PATH_TO_BIOME_NLCD_DATA.csv