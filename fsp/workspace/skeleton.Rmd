---
title: "MOD Data Quality Review, `r params$titleDate`"
author: "Your Name"
...
# Load portal data - SLS (subset of sites)
soil <- try(neonUtilities::loadByProduct(
  dpID='DP1.10086.001',
  check.size=F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
...
# Load portal data - BBC
roots <- try(neonUtilities::loadByProduct(
  dpID = 'DP1.10067.001',
  check.size = FALSE,
  site = "all",
  startdate = startDate_checked,
...
# Load portal data - ASI
surfaceWaterIsotopes <- try(neonUtilities::loadByProduct(
  dpID='DP1.20206.001',
  check.size=F, 
  site = c("POSE","LIRO", "PRPO", "SYCA", "MCRA", "CARI"),
  startdate = startDate_checked,
  enddate = endDate_checked,
  include.provisional = T,
  release = "LATEST",
...
# Load portal data - SWC
surfaceWaterChem <- try(neonUtilities::loadByProduct(
  dpID='DP1.20093.001',
  check.size=F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
...
# Load portal data - AGW
agwData <- try(neonUtilities::loadByProduct(
  dpID='DP1.20099.001',
  check.size=F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
...
# Load portal data - REA
rearation <- try(neonUtilities::loadByProduct(
  dpID='DP1.20190.001',
  check.size=F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
...
# Load portal data - MAM
mammals <- try(neonUtilities::loadByProduct(
  dpID='DP1.10072.001',
  check.size=F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
  site = c("OSBS", "WOOD", "SRER", "HARV", "TREE"),
...
# Load portal data - INV
inverts <- try(neonUtilities::loadByProduct(
  dpID='DP1.20120.001',
  check.size= F, 
  startdate = startDate_checked,
  enddate = endDate_checked,
...
# Load portal data - ASC
sediment <- try(neonUtilities::loadByProduct(
  dpID='DP1.20194.001',
  check.size=F,
  startdate = startDate_checked,
  enddate = endDate_checked,
...
if(is.null(attr(canopyFoliage, "class"))){
  ...
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(soil, "class"))){
  invisible(list2env(soil, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(roots, "class"))){
  invisible(list2env(roots, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(surfaceWaterIsotopes, "class"))){
  invisible(list2env(surfaceWaterIsotopes, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(surfaceWaterChem, "class"))){
  invisible(list2env(surfaceWaterChem, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(agwData, "class"))){
  invisible(list2env(agwData, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(rearation, "class"))){
  invisible(list2env(rearation, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(mammals, "class"))){
  invisible(list2env(mammals, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(inverts, "class"))){
  invisible(list2env(inverts, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

if(is.null(attr(sediment, "class"))){
  invisible(list2env(sediment, envir=.GlobalEnv))
}else{
  print("There are no data in this date range to check, no script outputs generated.")
  knitr::knit_exit()
}

# remove params list from environment
rm(params) 
```
...
cfc_fieldData_dupCheck <- neonOS::removeDups(data = cfc_fieldData,
                                             variables = variables_10026,
                                             table = 'cfc_fieldData') 

# Create 3 DFs: no dups, resolvable , unresolvable
cfc_fieldData_0 <- dplyr::filter(cfc_fieldData_dupCheck, duplicateRecordQF == "0")
cfc_fieldData_1 <- dplyr::filter(cfc_fieldData_dupCheck, duplicateRecordQF == "1")
cfc_fieldData_2 <- dplyr::filter(cfc_fieldData_dupCheck, duplicateRecordQF == "2")

# For unresolvable, get rid of duplicates (if any)
cfc_fieldData_2_keep <- cfc_fieldData_2 %>%
  # group by whatever fields are in the primary key
  dplyr::group_by(plotID, collectDate, subplotID, sampleID, sampleCode) %>% 
...
cfc_fieldData_noDups <- dplyr::bind_rows(cfc_fieldData_0, cfc_fieldData_1, cfc_fieldData_2_keep)

# Create objects for how many dups, if any this needs follow-up
dups1 <- nrow(cfc_fieldData_1)
dups2 <- nrow(cfc_fieldData_2)

# Run dup check function - SLS
sls_soilCoreCollection_dupCheck <- neonOS::removeDups(data = sls_soilCoreCollection,
                                                      variables = variables_10086,
                                                      table = 'sls_soilCoreCollection') 

# Create 3 DFs: no dups, resolvable , unresolvable
sls_soilCoreCollection_0 <- dplyr::filter(sls_soilCoreCollection_dupCheck, duplicateRecordQF == "0")
sls_soilCoreCollection_1 <- dplyr::filter(sls_soilCoreCollection_dupCheck, duplicateRecordQF == "1")
sls_soilCoreCollection_2 <- dplyr::filter(sls_soilCoreCollection_dupCheck, duplicateRecordQF == "2")

# For unresolvable, get rid of duplicates (if any)
sls_soilCoreCollection_2_keep <- sls_soilCoreCollection_2 %>%
  # group by primary key fields
  dplyr::group_by(sampleID, sampleCode) %>% 
  # only keep the first instance
  dplyr::filter(row_number()==1)

# Join now-resolved dups back with the other data for use in completeness checks
sls_soilCoreCollection_noDups <- dplyr::bind_rows(sls_soilCoreCollection_0, sls_soilCoreCollection_1, sls_soilCoreCollection_2_keep)

# Run dup check function - BBC
bbc_percore_dupCheck <- neonOS::removeDups(
  data = bbc_percore,
  variables = variables_10067,
  table = 'bbc_percore'
)

# Create 3 DFs: no dups, resolvable , unresolvable
bbc_percore_0 <- dplyr::filter(bbc_percore_dupCheck, duplicateRecordQF == "0")
bbc_percore_1 <- dplyr::filter(bbc_percore_dupCheck, duplicateRecordQF == "1")
bbc_percore_2 <- dplyr::filter(bbc_percore_dupCheck, duplicateRecordQF == "2")

bbc_percore_2_keep <- bbc_percore_2 %>%
    # group by primary key fields
  dplyr::group_by(plotID,samplingImpractical,sampleID) %>% 
    # only keep the first instance
  dplyr::filter(row_number()==1) 

# Join now-resolved dups back with the other data for use in completeness checks
bbc_percore_noDups <- dplyr::bind_rows(bbc_percore_0, bbc_percore_1, bbc_percore_2_keep)

# Run dup check function - REA
rea_widthFieldData_dupCheck <- neonOS::removeDups(
  data=rea_widthFieldData,
  variables=variables_20190,
  table='rea_widthFieldData')

# Create 3 DFs: no dups, resolvable , unresolvable
rea_widthFieldData_0 <- dplyr::filter(rea_widthFieldData_dupCheck, duplicateRecordQF == "0")
rea_widthFieldData_1 <- dplyr::filter(rea_widthFieldData_dupCheck, duplicateRecordQF == "1")
rea_widthFieldData_2 <- dplyr::filter(rea_widthFieldData_dupCheck, duplicateRecordQF == "2")

rea_widthFieldData_2_keep <- rea_widthFieldData_2 %>%
    # group by primary key fields
  dplyr::group_by(namedLocation, collectDate, widthMeasurementNumber) %>% 
    # only keep the first instance
  dplyr::filter(row_number()==1) 

# Join now-resolved dups back with the other data for use in completeness checks
rea_widthFieldData_noDups <- dplyr::bind_rows(rea_widthFieldData_0, rea_widthFieldData_1, rea_widthFieldData_2_keep)

# Run dup check function - MAM
mam_pertrapnight.tagsOnly <- filter(mam_pertrapnight, !is.na(tagID))# filter out records with no tagIDs (no captures)

mam_pertrapnight_dupCheck <- removeDups(
  data=mam_pertrapnight.tagsOnly,
  variables=variables_10072,
  table='mam_pertrapnight')

# Create 3 DFs: no dups, resolvable , unresolvable
mam_pertrapnight_0 <- dplyr::filter(mam_pertrapnight_dupCheck, duplicateRecordQF == "0")
mam_pertrapnight_1 <- dplyr::filter(mam_pertrapnight_dupCheck, duplicateRecordQF == "1")
mam_pertrapnight_2 <- dplyr::filter(mam_pertrapnight_dupCheck, duplicateRecordQF == "2")

mam_pertrapnight_2_keep <- mam_pertrapnight_2 %>%
    # group by primary key fields
  dplyr::group_by(plotID, trapCoordinate, collectDate, tagID, individualCode) %>% 
    # only keep the first instance
  dplyr::filter(row_number()==1) 

# Join now-resolved dups back with the other data for use in completeness checks
mam_pertrapnight_noDups <- dplyr::bind_rows(mam_pertrapnight_0, mam_pertrapnight_1, mam_pertrapnight_2_keep)
...
cfc_fieldData_noDups <- cfc_fieldData_noDups %>%
  dplyr::mutate(dpID = "DP1.10026.001", 
                dpName = "Plant foliar traits", 
                tableName = "cfc_fieldData")

sls_soilCoreCollection <- sls_soilCoreCollection %>%
  dplyr::mutate(dpID = "DP1.10086.001", 
                dpName = "Soil physical and chemical properties, periodic", 
                tableName = "sls_soilCoreCollection")

sls_soilCoreCollection_noDups <- sls_soilCoreCollection_noDups %>%
  dplyr::mutate(dpID = "DP1.10086.001", 
                dpName = "Soil physical and chemical properties, periodic", 
                tableName = "sls_soilCoreCollection")

sls_soilpH <- sls_soilpH %>%
  dplyr::mutate(dpID = "DP1.10086.001", 
                dpName = "Soil physical and chemical properties, periodic", 
                tableName = "sls_soilpH")

bbc_percore <-  bbc_percore %>%
  dplyr::mutate(dpID = "DP1.10067.001", 
                dpName = "Root biomass and chemistry, periodic", 
                tableName = "bbc_percore",
                year = substr(collectDate, 1, 4)) # need this make an eventID)

bbc_percore_noDups <-  bbc_percore_noDups %>%
  dplyr::mutate(dpID = "DP1.10067.001", 
                dpName = "Root biomass and chemistry, periodic", 
                tableName = "bbc_percore", 
                year = substr(collectDate, 1, 4)) # need this make an eventID

asi_fieldSuperParent <- asi_fieldSuperParent %>%
  dplyr::mutate(tableName="asi_fieldSuperParent",
                dpID="DP1.20206.001",
                dpName="Stable isotopes in surface water")

asi_fieldData <- asi_fieldData %>%
  dplyr::mutate(tableName="asi_fieldData",
                dpID="DP1.20206.001",
                dpName="Stable isotopes in surface water")

asi_POMExternalLabDataPerSample <- asi_POMExternalLabDataPerSample %>%
  dplyr::mutate(tableName="asi_POMExternalLabDataPerSample",
                dpID="DP1.20206.001",
                dpName="Stable isotopes in surface water")

asi_externalLabH2OIsotopes <- asi_externalLabH2OIsotopes %>%
  dplyr::mutate(tableName="asi_externalLabH2OIsotopes",
                dpID="DP1.20206.001",
                dpName="Stable isotopes in surface water")
...
ntr_internalLab <- ntr_internalLab %>%
  dplyr::mutate(dpID = "DP1.10086.001", 
                dpName = "Soil physical and chemical properties, periodic", 
                tableName = "ntr_internalLab")

ntr_externalLab <- ntr_externalLab %>%
  dplyr::mutate(dpID = "DP1.10086.001", 
                dpName = "Soil physical and chemical properties, periodic", 
                tableName = "ntr_externalLab")

swc_fieldSuperParent <- swc_fieldSuperParent%>%
  dplyr::mutate(tableName="swc_fieldSuperParent",
                dpID="DP1.20093.001",
                dpName="Chemical properties of surface water")

swc_externalLabDataByAnalyte <- swc_externalLabDataByAnalyte%>%
  dplyr::mutate(tableName="swc_externalLabDataByAnalyte",
                dpID="DP1.20093.001",
                dpName="Chemical properties of surface water")

# Add important metadata variables for remaining checks
agw_groundwaterFieldData <- agw_groundwaterFieldData %>%
  dplyr::mutate(tableName="agw_groundwaterFieldData",
                dpID="DP1.20099.001",
                dpName="Groundwater and Active Layer Measurements at Permafrost Sites")

# Need to create a primary key field, namedLocation + widthMeasurementNumber - plus add metadata
rea_widthFieldData_noDups <- rea_widthFieldData_noDups %>%
  dplyr::mutate(siteWidth = paste0(namedLocation, '.', widthMeasurementNumber), 
                dpID = "DP1.20190.001", 
                dpName = "Reaeration field and lab collection", 
                tableName = "rea_widthFieldData")

mam_pertrapnight_noDups <- mam_pertrapnight_noDups %>%
  dplyr::mutate(dpID = "DP1.10072.001", 
                dpName = "Small mammal box trapping", 
                tableName = "mam_pertrapnight")

inv_fieldData <- inv_fieldData %>%
  dplyr::mutate(dpID = "DP1.20120.001", 
                dpName = "Macroinvertebrate collection", 
                tableName = "inv_fieldData")

inv_taxonomyProcessed <- inv_taxonomyProcessed %>%
  dplyr::mutate(dpID = "DP1.20120.001", 
                dpName = "Macroinvertebrate collection", 
                tableName = "inv_taxonomyProcessed")

asc_fieldDataStation <- asc_fieldDataStation%>%
...
sls1 <- neonOSqc::complete_bout(
  input.df = sls_soilCoreCollection_noDups,
  mod = "sls",
...
DT::datatable(sls1$complete_bout_summary,
              extensions = "Buttons",
              caption = glue::glue("Summary table of 'complete_bout' results for 'sls_soilCoreCollection' records between {startDate_checked} and {endDate_checked}."),
              options = dtOptions)
```

#### Flags
```{r bout completeness flags table, eval = annual}
if(nrow(sls1$complete_bout_flags) > 0){
  DT::datatable(sls1$complete_bout_flags,
                extensions = "Buttons",
                caption = glue::glue( "Flags table of 'complete_bout' results for 'sls_soilCoreCollection' records between {startDate_checked} and {endDate_checked}."),
                options = dtOptions)
  } else {
...
bbc.2.plots <- neonOSqc::complete_within_bout(
...
If this is a monthly script, interpret flags with caution since bouts can take a few weeks and thus might span two months. For this reason, qcMetrics is T for annual runs but F for monthly runs. Examining flags in both plots and records might help with this. If flags exist in both tables, it could mean some plots were sampled in an adjacent month, whereas flags in the records table but not plots table more likely means the staff forgot to create sampling impractical records.
