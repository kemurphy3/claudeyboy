...
library(dotenv)
dotenv::load_dotenv('.Renviron')
github_pat <- Sys.getenv('GITHUB_PAT')
if (github_pat == "") stop("GitHub PAT not found in .env file!")
...
# Uses fsp_complete_bout.csv as lookup
if (file.exists('fsp_complete_bout.csv')) {
  fsp_bout_lut <- read.csv('fsp_complete_bout.csv', stringsAsFactors = FALSE)
  # Example: neonOSqc::complete_bout usage (requires correct input.df and LUT)
  ...
  # To run this check, provide fsp_complete_bout.csv in the working directory.
}
```
...
if (file.exists('fsp_within_bout_nums.csv')) {
  ...
  # Example: neonOSqc::complete_within_bout usage
  # Uncomment and adapt as needed:
  # fsp_within_bout_check <- neonOSqc::complete_within_bout(
  #   input.df = fsp_spectralData_noDups,
  #   mod = 'fsp',
  #   event.var = c('eventID'),
  #   table.name = 'fsp_spectralData',
  #   type = 'bands per sample',
  #   lut = fsp_within_bout_lut
  # )
} else {
  # Missing lookup table
  # To run this check, provide fsp_within_bout_nums.csv in the working directory.
}
```

# 3. Completeness: All required fields filled (example for fsp_boutMetadata)
```{r fsp_within_rec_bout, eval=TRUE}
# Example: neonOSqc::complete_within_rec usage
# Uncomment and adapt as needed:
# fsp_within_rec_bout_check <- neonOSqc::complete_within_rec(
#   input.df = fsp_boutMetadata_noDups,
#   id.var = 'eventID',
#   # Add other required arguments as needed
# )
```

# 4. Completeness: Cross-table completeness (sampleIDs match collection list)
```{r fsp_cross_table, eval=TRUE}
# Example: neonOSqc::complete_cross_table usage
# Uncomment and adapt as needed:
...
# 5. Timeliness: Bout completed within 31 days
```{r fsp_timely_bout_duration, eval=TRUE}
# Example: neonOSqc::timely_bout_duration usage
# Uncomment and adapt as needed:
# fsp_timely_bout_duration_check <- neonOSqc::timely_bout_duration(
...
# 6. Plausibility: Duplicate checks
```{r fsp_duplicates, eval=TRUE}
# Example: neonOS::removeDups usage
# Uncomment and adapt as needed:
# fsp_boutMetadata_dupCheck <- neonOS::removeDups(data = fsp_boutMetadata, variables = eventID, table = 'fsp_boutMetadata')
# fsp_sampleMetadata_dupCheck <- neonOS::removeDups(data = fsp_sampleMetadata, variables = sampleID, table = 'fsp_sampleMetadata')
# fsp_spectralData_dupCheck <- neonOS::removeDups(data = fsp_spectralData, variables = spectralSampleID, table = 'fsp_spectralData')
```

# 7. Plausibility: Reflectance and wavelength range checks (custom)
```{r fsp_spectral_range_checks, eval=TRUE}
if (file.exists('FSP_HARV_20240808_182820250724T192341.973Z.csv')) {
  fsp_spectra <- read.csv('FSP_HARV_20240808_182820250724T192341.973Z.csv', stringsAsFactors = FALSE)
  # Check reflectance range
  reflectance_out_of_range <- subset(fsp_spectra, reflectance < 0 | reflectance > 1)
  # Check wavelength range
  wavelength_out_of_range <- subset(fsp_spectra, wavelength < 0 | wavelength > 3000)
  # Output results
  print(paste('Reflectance out of range:', nrow(reflectance_out_of_range)))
  print(paste('Wavelength out of range:', nrow(wavelength_out_of_range)))
} else {
  # Missing spectral data file
  # To run this check, provide FSP_HARV_20240808_182820250724T192341.973Z.csv in the working directory.
}
```
...
if (file.exists('FSP_HARV_20240808_182820250724T192341.973Z.csv')) {
  fsp_spectra <- read.csv('FSP_HARV_20240808_182820250724T192341.973Z.csv', stringsAsFactors = FALSE)
  # If there is a date column, use it. If not, extract from spectralSampleID or filename if possible.
  # Here, we assume the date is encoded in the spectralSampleID as YYYYMMDD (e.g., FSP_HARV_20240808_1828)
  library(dplyr)
  library(stringr)
  fsp_spectra$date <- str_extract(fsp_spectra$spectralSampleID, '\\d{8}')
  fsp_spectra$date <- as.Date(fsp_spectra$date, format='%Y%m%d')
  # For each unique spectralSampleID, get min and max date
  sample_ranges <- fsp_spectra %>%
    group_by(spectralSampleID) %>%
    summarize(first_date = min(date, na.rm=TRUE), last_date = max(date, na.rm=TRUE)) %>%
    mutate(days_between = as.numeric(last_date - first_date))
  # Flag any with > 30 days
  flagged <- sample_ranges %>% filter(days_between > 30)
  print(sample_ranges)
  print(flagged)
  cat('Number of samples with >30 days between first and last record:', nrow(flagged), '\n')
} else {
  # Missing spectral data file
  # To run this check, provide FSP_HARV_20240808_182820250724T192341.973Z.csv in the working directory.
}
...
s```{r fsp_n_gt_1_per_eventID, eval=TRUE}
if (file.exists('Field_spectra_data,_Level_0_fsp_sampleMetadata_in.txt') && file.exists('Field_spectra_data,_Level_0_fsp_boutMetadata_in.txt')) {
  sample_meta <- read.delim('Field_spectra_data,_Level_0_fsp_sampleMetadata_in.txt', stringsAsFactors = FALSE)
  bout_meta <- read.delim('Field_spectra_data,_Level_0_fsp_boutMetadata_in.txt', stringsAsFactors = FALSE)
  # For each eventID in bout_meta, check if there is at least one sample in sample_meta
  event_counts <- table(sample_meta$eventID)
  bout_eventIDs <- unique(bout_meta$eventID)
  missing_events <- bout_eventIDs[!bout_eventIDs %in% names(event_counts)]
  cat('Number of eventIDs in bout metadata with at least one sample:', sum(bout_eventIDs %in% names(event_counts)), '\n')
  cat('Number of eventIDs in bout metadata with NO samples:', length(missing_events), '\n')
  if (length(missing_events) > 0) {
    print(missing_events)
  }
} else {
  # Missing sample or bout metadata file
  # To run this check, provide both Field_spectra_data,_Level_0_fsp_sampleMetadata_in.txt and Field_spectra_data,_Level_0_fsp_boutMetadata_in.txt in the working directory.
}
```
...
if (file.exists('FSP_HARV_20240808_182820250724T192341.973Z.csv')) {
  spectra <- read.csv('FSP_HARV_20240808_182820250724T192341.973Z.csv', stringsAsFactors = FALSE)
  library(dplyr)
  # Use a window of +/- 5 nm around 500 and 1000
  window_500 <- 495:505
  window_1000 <- 995:1005
  ratio_results <- spectra %>%
    group_by(spectralSampleID) %>%
    summarize(
      avg_500 = mean(reflectance[wavelength %in% window_500], na.rm=TRUE),
      avg_1000 = mean(reflectance[wavelength %in% window_1000], na.rm=TRUE)
    ) %>%
    mutate(ratio_check = avg_1000 > avg_500)
  n_failed <- sum(!ratio_results$ratio_check, na.rm=TRUE)
  cat('Number of samples where reflectance at ~1000 nm is NOT greater than at ~500 nm:', n_failed, '\n')
  if (n_failed > 0) {
    print(ratio_results[!ratio_results$ratio_check,])
  }
} else {
  # Missing spectral data file
  # To run this check, provide FSP_HARV_20240808_182820250724T192341.973Z.csv in the working directory.
...
# Assume fsp_spectralData_meta is loaded and contains downloadFileURL
if (exists('fsp_spectralData_meta') && 'downloadFileURL' %in% colnames(fsp_spectralData_meta)) {
  band_count_results <- list()
  reflectance_range_results <- list()
  wavelength_range_results <- list()
  for (i in seq_len(nrow(fsp_spectralData_meta))) {
    file_url <- fsp_spectralData_meta$downloadFileURL[i]
    sample_id <- fsp_spectralData_meta$spectralSampleID[i]
